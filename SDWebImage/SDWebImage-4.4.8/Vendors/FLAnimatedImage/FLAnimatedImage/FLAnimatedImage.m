...
@property (nonatomic, assign, readonly) NSUInteger frameCacheSizeOptimal; // The optimal number of frames to cache based on image size & number of frames; never changes 缓存大小的优化
...
@property (nonatomic, strong, readonly) FLAnimatedImage *weakProxy; // 代理
...
static NSHashTable *allAnimatedImagesWeak; // 不持有对象
...
//     下面是初始化基本的数据内容
    ...
    if (self) {
        ...
        _data = data;
        _predrawingEnabled = isPredrawingEnabled; // 是否预先绘制
        
        // Initialize internal data structures
        _cachedFramesForIndexes = [[NSMutableDictionary alloc] init]; // 缓存索引对应的内容
        _cachedFrameIndexes = [[NSMutableIndexSet alloc] init]; // 缓存帧索引
        _requestedFrameIndexes = [[NSMutableIndexSet alloc] init]; // 请求帧索引
...
//         创建一个图像的句柄
        ...
        BOOL isGIFData = UTTypeConformsTo(imageSourceContainerType, kUTTypeGIF); // 能不能够转化为对应的类型
        if (!isGIFData) {
        ...
        for (size_t i = 0; i < imageCount; i++) {
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        // Set poster image
                        if (!self.posterImage) { // 这样一般就能够设置到第一张的图片
                            ...
                            [self.cachedFramesForIndexes setObject:self.posterImage forKey:@(self.posterImageFrameIndex)]; //设置的是我们预览图片
                            [self.cachedFrameIndexes addIndex:self.posterImageFrameIndex];
//                             预览图片索引
                        }
                        
                        // Get `DelayTime` 获取延迟时间
                        ...
                        if (delayTime == nil) {
                            if (i == 0) {
                                ...
                                delayTime = @(kDelayTimeIntervalDefault); // 默认时间
                            } else {
                                FLLog(FLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties); // 上一帧时间
                        ...
                        if ([delayTime floatValue] < ((float)kFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) { // 不太明白为什么要做这个操作
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            CGFloat animatedImageDataSize = CGImageGetBytesPerRow(self.posterImage.CGImage) * self.size.height * (self.frameCount - skippedFrameCount) / MEGABYTE;// 获取的是M的数据
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                if (image && weakSelf) {
                    dispatch_async(dispatch_get_main_queue(), ^{ // 这里添加了缓存
