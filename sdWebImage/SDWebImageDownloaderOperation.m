// SDWebImageDownloaderOperation下载的操作的实例
 ...
 */
// 如果想实现爱你一个downloader， 需要继承NSOPeration 然后实现SDWebImageDownloaderOperation 这个接口
...
- (BOOL)cancel:(nullable id)token; // 取消

@property (strong, nonatomic, readonly, nullable) NSURLRequest *request; // 请求
@property (strong, nonatomic, readonly, nullable) NSURLResponse *response; // 响应

@optional
@property (strong, nonatomic, readonly, nullable) NSURLSessionTask *dataTask; // 数据任务
@property (strong, nonatomic, readonly, nullable) NSURLSessionTaskMetrics *metrics API_AVAILABLE(macosx(10.12), ios(10.0), watchos(3.0), tvos(10.0)); // 流量监控
@property (strong, nonatomic, nullable) NSURLCredential *credential; // 验证
@property (assign, nonatomic) double minimumProgressInterval; // 最小的进度间隔
...
@property (assign, nonatomic, readonly) SDWebImageDownloaderOptions options;  // 图片下载选项
@property (copy, nonatomic, readonly, nullable) SDWebImageContext *context; // 上下文
...
- (BOOL)cancel:(nullable id)token; // nsPeration， 基本上我们写一个operation的时候， 都是要重写这个方法

@end


//  token 这个内容和doloader 有什么关系， 为什么这个还是继承了这个东西， 这个处理的结果是怎么样的？ downloader 应该是一个中的下载器， operation是单个的下载器
...
- (void)callCompletionBlocksWithImage:(nullable UIImage *)image
                            imageData:(nullable NSData *)imageData
                             ...
                             finished:(BOOL)finished {
                                //   将所有的会带哦都回调回去 ，为什么我们请求的时候怎么知道哪个是哪个的数据呢？ 就是通过这个diction来进行处理的key、value
