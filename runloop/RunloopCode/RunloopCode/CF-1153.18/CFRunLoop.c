...
// USE_DISPATCH_SOURCE_FOR_TIMERS  也就是sourceTimer是在macox上才会有的
#if DEPLOYMENT_TARGET_MACOSX
#define USE_DISPATCH_SOURCE_FOR_TIMERS 1
#define USE_MK_TIMER_TOO 1
#else
#define USE_DISPATCH_SOURCE_FOR_TIMERS 0
#define USE_MK_TIMER_TOO 1 // 这个不知道是干嘛的？ 应该是核心的时钟
#endif
...
struct __CFRunLoopMode {
    ...
    CFMutableDictionaryRef _portToV1SourceMap; // port转化为source1的字典
...
static CFRunLoopRef __CFRunLoopCreate(pthread_t t) {
    ...
    __CFRunLoopLockInit(&loop->_lock); // runloop初始化
    loop->_wakeUpPort = __CFPortAllocate(); // 启动port
    if (CFPORT_NULL == loop->_wakeUpPort) HALT;
    __CFRunLoopSetIgnoreWakeUps(loop);
    loop->_commonModes = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks); //设置commonmode
...
static CFMutableDictionaryRef __CFRunLoops = NULL; // 用来存储线程runloop的字典
static CFLock_t loopsLock = CFLockInit; // runloop的锁

// should only be called by Foundation
// t==0 is a synonym for "main thread" that always works
// 不管是主线程获取runloop还是当前线程获取runloop ，都是要调用这个方法的
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    if (pthread_equal(t, kNilPthreadT)) {
        t = pthread_main_thread_np();
    }
    __CFLock(&loopsLock);
    if (!__CFRunLoops) { // 不存在就创建一个字典，同事创建主线程的runloop
        __CFUnlock(&loopsLock);
        CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np()); // 创建了runloop
        CFDictionarySetValue(dict, pthreadPointer(pthread_main_thread_np()), mainLoop); // 线程为key， runloop 为value
    ...
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t)); // 获取字典里面的runloop
    __CFUnlock(&loopsLock);
    if (!loop) { // 没有runloop就创建，并将它存放到字典里面
        CFRunLoopRef newLoop = __CFRunLoopCreate(t); // 创建runloop 上下文
        __CFLock(&loopsLock);
        
//         感觉这里判断多了， 直接设置进去就好了，可能是规避问题吧
        loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t)); // 这个时候发现字典里面有loop
        if (!loop) {
            CFDictionarySetValue(__CFRunLoops, pthreadPointer(t), newLoop); // 设置到字典里面
            loop = newLoop;
        }
    ...
    if (pthread_equal(t, pthread_self())) { // 如果请求的线程是当前的线程， 设置有关的缓存，有利于快速获取
...
//     获取主线程的runloop 使用了_CFRunLoopGet0() 方法
...
CFRunLoopRef CFRunLoopGetCurrent(void) {
    CHECK_FOR_FORK();
//     这里应该是有有缓存的， 从TSD 里面后去
    CFRunLoopRef rl = (CFRunLoopRef)_CFGetTSD(__CFTSDKeyRunLoop); // 也就是缓存的内容
    if (rl) return rl;
//     获取当期那的线程的方法
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    do {
        ...
        __CFRunLoopUnsetIgnoreWakeUps(rl); // 这里忽略掉睡眠
        ...
        __CFRunLoopDoBlocks(rl, rlm); // 处理block
        
        //         处理source0
        Boolean sourceHandledThisLoop = __CFRunLoopDoSources0(rl, rlm, stopAfterHandle);
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm); // runloop处理块
        }
...
//         这里是port转为source1
...
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
#if USE_DISPATCH_SOURCE_FOR_TIMERS
        // source作为定时器， 这个时候就要调用了， 这样做会不会影响到什么？
        do {
        ...
        msg = (mach_msg_header_t *)msg_buffer;
        __CFRunLoopServiceMachPort(waitSet, &msg, sizeof(msg_buffer), &livePort, poll ? 0 : TIMEOUT_INFINITY, &voucherState, &voucherCopy); // 服务port的内容
#endif
        
        
#elif DEPLOYMENT_TARGET_WINDOWS // windows系统的内容
...
#if USE_DISPATCH_SOURCE_FOR_TIMERS ， 在mac上才会有的
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished; // 销毁中,执行完成
    __CFRunLoopLock(rl); // 加锁
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false); //mode
    if (NULL == currentMode || __CFRunLoopModeIsEmpty(rl, currentMode, rl->_currentMode)) {
        Boolean did = false;
        if (currentMode) __CFRunLoopModeUnlock(currentMode);
        __CFRunLoopUnlock(rl);
        return did ? kCFRunLoopRunHandledSource : kCFRunLoopRunFinished;
    // 看看是empty/NULL 若果是，就返回处理完成或者source
    /// 进入loop时参数说处理完事件就返回。
    ...
    if (currentMode->_observerMask & kCFRunLoopEntry )
        __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry); //runloop进入
    
    result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode); // 这里是处理source的逻辑
    
    if (currentMode->_observerMask & kCFRunLoopExit )
        __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopExit); // rooloop 进入退出状态
