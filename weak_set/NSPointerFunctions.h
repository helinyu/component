...
/*
    NSPointerFunctions 
    订了描述函数的合适指针引用
    使用 NSHashTable, NSMapTable, and NSPointerArray ，此对象定义提供给这些集合对象的指针的获取和保留行为。
    函数被分为两个族， 这些定义个人的， 例如： object 或 cString 和那些 描述内存管理问题，例如内存销毁。 
    共同的个人和内存管理者分区被提供作为枚举， 
    pointer集合拷贝NSPointerFunctions 对象输入和输出， 所以，对于子类是无效的。
    未来自定义提供通过方法，例如： 可以在未来扩展
*/
...
typedef NS_OPTIONS(NSUInteger, NSPointerFunctionsOptions) {
    ...
    // use strong write-barrier to backing store; use GC memory on copyIn 缺省值，在 CG 和 MRC 下强引用成员
    ...
    // use -hash and -isEqual, object description  //hash、isEqual、对象描述
    NSPointerFunctionsOpaquePersonality API_AVAILABLE(macos(10.5), ios(6.0), watchos(2.0), tvos(9.0)) = (1UL << 8),        
     // use shifted pointer hash and direct equality  pointer 的 hash 、直接判等
    NSPointerFunctionsObjectPointerPersonality API_AVAILABLE(macos(10.5), ios(6.0), watchos(2.0), tvos(9.0)) = (2UL << 8),  
    // use shifted pointer hash and direct equality, object description pointer 的 hash、直接判等、对象描述
    NSPointerFunctionsCStringPersonality API_AVAILABLE(macos(10.5), ios(6.0), watchos(2.0), tvos(9.0)) = (3UL << 8),        
    // use a string hash and strcmp, description assumes UTF-8 contents; recommended for UTF-8 (or ASCII, which is a subset) only cstrings  string 的 hash、strcmp 函数、UTF-8 编码方式的描述
    NSPointerFunctionsStructPersonality API_AVAILABLE(macos(10.5), ios(6.0), watchos(2.0), tvos(9.0)) = (4UL << 8),         
    // use a memory hash and memcmp (using size function you must set) 内存 hash、memcmp 函数
    NSPointerFunctionsIntegerPersonality API_AVAILABLE(macos(10.5), ios(6.0), watchos(2.0), tvos(9.0)) = (5UL << 8),        
    // use unshifted value as hash & equality   值的 hash
...
@property (nullable) NSUInteger (*sizeFunction)(const void *item); // 函数大小
@property (nullable) NSString * _Nullable (*descriptionFunction)(const void *item); // 描述
...
// https://www.cnblogs.com/lxlx1798/p/6651949.html  参考使用
...
/*
   ...
   Example
       Lets say you have a source of C Strings that often repeat and you need to provide unique NSString counterparts. 
       For this you likely want a NSMapTable with strong copy-in C Strings as the keys and weak NSString values.  
       As long as the NSString counterparts are in use, they stay in the table and don't need to be created each time they show up.
...
*/
#endif 
// defined __FOUNDATION_NSPOINTERFUNCTIONS__
