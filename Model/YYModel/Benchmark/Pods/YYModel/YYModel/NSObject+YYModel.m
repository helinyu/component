...
@interface _YYModelMeta : NSObject {
    ...
    NSDictionary *_mapper; // 这里包括了key以及keypath上的内容
    /// Array<_YYModelPropertyMeta>, all property meta of this model.
    NSArray *_allPropertyMetas; // 所有的属性
    /// Array<_YYModelPropertyMeta>, property meta which is mapped to a key path.
    NSArray *_keyPathPropertyMetas; // keypath对应的属性
    /// Array<_YYModelPropertyMeta>, property meta which is mapped to multi keys.
    NSArray *_multiKeysPropertyMetas; // 一个属性对应多个key
    /// The number of mapped key (and key path), same to _mapper.count.
    NSUInteger _keyMappedCount; // key 映射的数目
    /// Model class type.
    YYEncodingNSType _nsType;
    
    BOOL _hasCustomWillTransformFromDictionary; // 从dictionary转化为model
    BOOL _hasCustomTransformFromDictionary; //从dictionary转化对象
    BOOL _hasCustomTransformToDictionary; // 从对象转化为model
    BOOL _hasCustomClassFromDictionary; // 从dictionary转化为对应的model
}
@end

@implementation _YYModelMeta
- (instancetype)initWithClass:(Class)cls {
    YYClassInfo *classInfo = [YYClassInfo classInfoWithClass:cls]; // 获取类的信息
    ...
    _hasCustomClassFromDictionary = ([cls respondsToSelector:@selector(modelCustomClassForDictionary:)]);// 初始化的时候就判断有没有这个定义的问题了
...
+ (instancetype)metaWithClass:(Class)cls {
    if (!cls) return nil;
    static CFMutableDictionaryRef cache; // 这个模型是否缓存过了？？
    static dispatch_once_t onceToken;
    ...
    _YYModelMeta *meta = CFDictionaryGetValue(cache, (__bridge const void *)(cls)); // 创建一个类的源模型对象
    dispatch_semaphore_signal(lock);
    if (!meta || meta->_classInfo.needUpdate) { // 元类是否一斤存在来缓存里面，并且类信息是否需要更新
...
typedef struct {
    void *modelMeta;  ///< _YYModelMeta // 对象的元类
    void *model;      ///< id (self) // 对象
    void *dictionary; ///< NSDictionary (json) // 数据源
} ModelSetContext;
 ...
 */
// 模型设置字典的内容 ， 这个方法就是给对应的属性赋值的操作
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ModelSetContext *context = _context;
    __unsafe_unretained _YYModelMeta *meta = (__bridge _YYModelMeta *)(context->modelMeta); // 元类
    __unsafe_unretained _YYModelPropertyMeta *propertyMeta = [meta->_mapper objectForKey:(__bridge id)(_key)]; // 元类具有的属性
    __unsafe_unretained id model = (__bridge id)(context->model); // 模型对象
    while (propertyMeta) {
        if (propertyMeta->_setter) { // 有对应的setter ， 这个setter表示可以设置值，对应我们里面的是什么？
            ModelSetValueForProperty(model, (__bridge __unsafe_unretained id)_value, propertyMeta); // 设置属性
        }
        propertyMeta = propertyMeta->_next; // 下一个属性 （是内部的还是。。。）
    };
}
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta->_mappedToKeyPath) { // keyPath 上的你内容
...
// 将有关的数据转化为dictonary ，为下一步解析提供了基础
...
+ (instancetype)yy_modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:cls]; //创建了模型类的元对象
    if (modelMeta->_hasCustomClassFromDictionary) {
        cls = [cls modelCustomClassForDictionary:dictionary] ?: cls;
    }
    
    NSObject *one = [cls new]; // 创建对应的对象，然后赋值
    if ([one yy_modelSetWithDictionary:dictionary]) return one; // 给这个对象设置值
    return nil;
}
...
- (BOOL)yy_modelSetWithDictionary:(NSDictionary *)dic {
    ...
    if (modelMeta->_hasCustomWillTransformFromDictionary) { // 这个是在转成model之前的一个数据替换
    ...
    if (modelMeta->_keyMappedCount >= CFDictionaryGetCount((CFDictionaryRef)dic)) {
        ...
        if (modelMeta->_keyPathPropertyMetas) { // keypath的属性精心赋值
            CFArrayApplyFunction((CFArrayRef)modelMeta->_keyPathPropertyMetas,
                                 CFRangeMake(0, CFArrayGetCount((CFArrayRef)modelMeta->_keyPathPropertyMetas)),
                                 ModelSetWithPropertyMetaArrayFunction,
                                 &context);
        }
        if (modelMeta->_multiKeysPropertyMetas) { // 多个key对应的一个属性
            CFArrayApplyFunction((CFArrayRef)modelMeta->_multiKeysPropertyMetas,
                                 CFRangeMake(0, CFArrayGetCount((CFArrayRef)modelMeta->_multiKeysPropertyMetas)),
                                 ModelSetWithPropertyMetaArrayFunction,
                                 &context);
        }
    } else { // 元类对应的属性没有比数据大的胡啊，可以直接就赋值初六了
    ...
    if (modelMeta->_hasCustomTransformFromDictionary) { // 处理完数据转为模型之后的额外处理
