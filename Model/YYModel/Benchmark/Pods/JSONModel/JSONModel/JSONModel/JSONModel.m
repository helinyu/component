...
//    }
    ...
    //run any custom model validation 可以不用管
...
// 处理字典的内容和keyMapper， 这个主要有的，都是需要去设置的
-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(JSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) { // 错误的内容
...
//
        ...
        if (property) {
            ...
            // 0.5) handle nils
            if (isNull(jsonValue)) { // 处理nil，这个清空的值
            ...
            if ([self __isJSONModelSubClass:property.type]) {
                ...
                id value = [[property.type alloc] initWithDictionary: jsonValue error:&initErr]; // 重新处理
                ...
                //  这里处理了自定义的集合的内容，通过protocol这个字段来进行处理
...
-(void)__inspectProperties
{
    ...
    while (class != [JSONModel class]) {
        ...
        objc_property_t *properties = class_copyPropertyList(class, &propertyCount); // 这里直接获取属性以及属性并不能的数目
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            NSArray* attributeItems = [propertyAttributes componentsSeparatedByString:@","]; // 有关属性的权限

            //ignore read-only properties
            if ([attributeItems containsObject:@"R"]) { // 属性是值读的时候，这个就不管了， 这个好像是有问题的
